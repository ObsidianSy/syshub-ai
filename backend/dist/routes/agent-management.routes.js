...
router.post('/systems/register', async (req, res) => {
    try {
        ...
        await agentService.registerSystem(systemId, config);
        res.json({
            success: true,
            systemId,
 ...
 */
router.get('/systems/:systemId/health', async (req, res) => {
    try {
        const { systemId } = req.params;
 ...
 */
router.get('/systems/:systemId/tables', async (req, res) => {
    try {
        const { systemId } = req.params;
 ...
 */
router.get('/systems/:systemId/tables/:tableName/schema', async (req, res) => {
    try {
 ...
 */
router.post('/systems/:systemId/query', async (req, res) => {
    try {
        const { systemId } = req.params;
        const schema = z.object({
            query: z.string(),
...
router.get('/sync/status/:jobId', async (req, res) => {
    try {
        ...
        if (!['sync', 'index', 'embedding'].includes(queue)) {
            return res.status(400).json({ error: 'Invalid queue name' });
        }
        const status = await queueService.getJobStatus(jobId, queue);
        if (!status) {
 ...
 * Retry a failed job
 */
router.post('/sync/retry/:jobId', async (req, res) => {
    try {
        const { jobId } = req.params;
        const schema = z.object({
 ...
 * Cancel a job
 */
router.delete('/sync/cancel/:jobId', async (req, res) => {
    try {
        const { jobId } = req.params;
        const { queue = 'sync' } = req.query;
        if (!['sync', 'index', 'embedding'].includes(queue)) {
        ...
        await queueService.cancelJob(jobId, queue);
        res.json({
            success: true,
...
//# sourceMappingURL=agent-management.routes.js.map