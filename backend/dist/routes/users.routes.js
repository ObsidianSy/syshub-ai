...
router.get('/me', async (req, res) => {
    try {
        ...
        const result = await query(`SELECT id, email, full_name, role, avatar_url, is_active, last_login, created_at
       FROM users
       WHERE id = $1`, [userId]);
...
router.put('/me', async (req, res) => {
    try {
       ...
       WHERE id = $3
       RETURNING id, email, full_name, role, avatar_url`, [fullName, avatarUrl, userId]);
        if (result.rows.length === 0) {
            return res.status(404).json({ error: 'Usuário não encontrado' });
        }
        res.json(result.rows[0]);
...
router.get('/', requireAdmin, async (req, res) => {
    try {
        const result = await query(`SELECT id, email, full_name as "fullName", role, is_active as "isActive", 
...
router.post('/', requireAdmin, async (req, res) => {
    try {
        ...
        // Criar usuário
        const result = await query(`INSERT INTO users (email, password_hash, full_name, role)
       VALUES ($1, $2, $3, $4)
       RETURNING id, email, full_name as "fullName", role, is_active as "isActive", created_at as "createdAt"`, [email, passwordHash, fullName, role]);
