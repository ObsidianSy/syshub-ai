...
router.get('/', async (req, res) => {
    try {
        ...
        const total = db.prepare('SELECT COUNT(*) as count FROM queries WHERE user_id = ?').get(userId);
        res.json({
            queries,
            total: total.count,
            limit: Number(limit),
...
router.get('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.user?.id;
        const query = db.prepare('SELECT * FROM queries WHERE id = ? AND user_id = ?').get(id, userId);
        if (!query) {
...
//# sourceMappingURL=queries-sqlite.routes.js.map