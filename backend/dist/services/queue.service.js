...
export const syncQueue = redisAvailable && connection ? new Queue('agent-sync', { ...defaultQueueOptions, connection }) : null;
export const indexQueue = redisAvailable && connection ? new Queue('agent-index', { ...defaultQueueOptions, connection }) : null;
export const embeddingQueue = redisAvailable && connection ? new Queue('agent-embedding', { ...defaultQueueOptions, connection }) : null;
...
const syncWorker = redisAvailable && connection ? new Worker('agent-sync', async (job) => {
...
const indexWorker = redisAvailable && connection ? new Worker('agent-index', async (job) => {
...
const embeddingWorker = redisAvailable && connection ? new Worker('agent-embedding', async (job) => {
...
export class QueueService {
     ...
     */
    async scheduleSyncJob(systemId, cronPattern, options) {
        if (!redisAvailable || !syncQueue) {
            throw new Error('Queue system not available - Redis not connected');
        }
        return syncQueue.add(`sync-system-${systemId}`, {
            systemId,
            tables: options?.tables,
            incrementalColumn: options?.incrementalColumn,
        }, {
     ...
     * Clean old jobs
     */
    async cleanQueue(queueName, grace = 86400000 // 24 hours in ms
    ) {
        if (!redisAvailable)
            return [];
        const queue = queueName === 'sync' ? syncQueue : queueName === 'index' ? indexQueue : embeddingQueue;
    ...
    async close() {
        ...
        const promises = [];
        if (syncWorker)
            promises.push(syncWorker.close());
        if (indexWorker)
            promises.push(indexWorker.close());
        if (embeddingWorker)
            promises.push(embeddingWorker.close());
        if (syncQueue)
            promises.push(syncQueue.close());
        if (indexQueue)
            promises.push(indexQueue.close());
        if (embeddingQueue)
            promises.push(embeddingQueue.close());
        if (connection)
