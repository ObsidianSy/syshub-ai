...
router.get('/:id', async (req: AuthRequest, res) => {
  try {
    ...
    const messagesResult = await query(
      `SELECT m.*, s.name as system_name, s.icon as system_icon
...
router.post('/:id/messages', async (req: AuthRequest, res) => {
  try {
    ...
    const result = await query(
      `INSERT INTO conversation_messages (conversation_id, role, content, system_id, metadata)
       VALUES ($1, $2, $3, $4, $5)
       RETURNING *`,
      [id, role, content, systemId, metadata ? JSON.stringify(metadata) : null]
    );
