 ...
 */
router.get("/:conversationId/documents", async (req: AuthRequest, res) => {
  try {
    ...
    const documentsWithUrls = (documents as DocRow[]).map((doc) => ({
      id: doc.id,
...
router.post(
  ...
  async (req: AuthRequest, res) => {
    try {
      ...
      let storagePath: string | null = file.path;
 ...
 */
router.get(
  "/:conversationId/documents/:documentId/download",
  async (req: AuthRequest, res) => {
    try {
      ...
      ).get(documentId, conversationId) as DocumentDownload | undefined;
...
router.delete(
  ...
  async (req: AuthRequest, res) => {
    try {
      ...
      // Buscar documento
      const document = db.prepare(
        "SELECT storage_path FROM conversation_documents WHERE id = ? AND conversation_id = ?"
      ).get(documentId, conversationId) as { storage_path: string | null } | undefined;
      ...
      // Deletar do banco
      db.prepare(
        "DELETE FROM conversation_documents WHERE id = ? AND conversation_id = ?"
      ).run(documentId, conversationId);
