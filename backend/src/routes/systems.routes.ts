...
router.get('/:id', async (req, res) => {
  try {
    ...
    const result = await query(
      'SELECT * FROM systems WHERE id = $1 AND is_active = true',
      [id]
    );
  ...
  try {
    ...
    const result = await query(
      `INSERT INTO systems (name, slug, category, status, description, icon, version)
       ...
       RETURNING *`,
      [data.name, data.slug, data.category, data.status, data.description, data.icon, data.version]
    );
  ...
  try {
    ...
    const result = await query(
      `UPDATE systems 
       ...
       WHERE id = $7
       RETURNING *`,
      [name, category, status, description, icon, version, id]
    );
...
router.delete('/:id', authenticateToken, async (req: AuthRequest, res) => {
  try {
    ...
    const result = await query(
      'UPDATE systems SET is_active = false WHERE id = $1 RETURNING *',
      [id]
    );
