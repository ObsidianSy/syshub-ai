  ...
  try {
    ...
    const result = await query(
      `INSERT INTO queries (user_id, question, system_id, system_name, status)
       ...
       RETURNING *`,
      [userId, question, systemId, systemName]
    );
    ...
    // Log no histórico
    await query(
      `INSERT INTO query_history (query_id, user_id, action, metadata)
       VALUES ($1, $2, 'created', $3)`,
      [newQuery.id, userId, JSON.stringify({ question })]
    );
...
router.get('/:id', async (req: AuthRequest, res) => {
  try {
    ...
    // Log de visualização
    await query(
      `INSERT INTO query_history (query_id, user_id, action)
       VALUES ($1, $2, 'viewed')`,
      [id, userId]
    );
...
router.post('/:id/favorite', async (req: AuthRequest, res) => {
  try {
    ...
    await query(
      `INSERT INTO query_history (query_id, user_id, action)
       VALUES ($1, $2, $3)`,
      [id, userId, isFavorite ? 'favorited' : 'unfavorited']
    );
